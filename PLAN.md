...
Target (replaces `campaign_faithful` of `f6/FAITHFULNESS_PLAN.md` §0 by
...
Files and lemmas (plan numbering from `f6/FAITHFULNESS_PLAN.md`; all
...
`lake env lean` before it is imported by `SmpF5.lean`.

Acceptance: `lake build` clean; `grep -rn sorry SmpF5/` empty;
...
1. **All 321,492 cube formulas re-hashed from Lean.** `export_cubes6
   ...
   `stopUnits`; `f6/lean_rehash.py` composes `header ‖ body ‖ units` with
...
- `STATUS.md`: state the missing theorem in the right direction (instance
...
- `README.md`: top bullet and f(6) row carry the caveat until A lands, then
  ...
  identity checks.
- `VERIFYING.md`: replace the 2026-09-01 f(6) section with the campaign
  recipe (gunzip, `--audit`, `export_cubes6 --final`, `lean_rehash.py`, the
  axiom list); fix the 240-file hash wording.
- `INSIGHTS.md`, `NOTES.md`, `f6/README.md`, `f6/CAMPAIGN.md`,
  `f6/FAITHFULNESS_PLAN.md` header/progress log/§8 table: certificate
  campaign is the evidence, enumeration is corroboration.
- Papers: `f6/paper/f6.tex` abstract, "Validation", "Lean" and outlook
  ...
  al. 2024) and 85 here; Lean counts refreshed. `f6/theory.tex` and
  `f5/paper/f5.tex`: CI wording, counts.
- `f7/README.md`: full-budget and multi-seed claims either backed by
...
- Remove `f5/lean/SmpF5/README.md` (Lake template). LICENSE and
