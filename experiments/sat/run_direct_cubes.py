"""Run order-5 direct-encoding cubes with kissat and drat-trim.

These are historical Python-encoded cubes. The final Lean-exported
selector cubes are verified by tools/verify_five_cubes.py instead.
Pass cube indices as positional arguments; the default runs all 120."""
...
HERE = os.path.join(ROOT, "runs", "f5", "direct-encoding")
os.makedirs(HERE, exist_ok=True)
DT = os.path.join(ROOT, "dt-src", "drat-trim")
...
def solve_cube(idx, order):
    ...
    base = os.path.join(HERE, f"cube{idx:03d}")
    cnf = base + ".cnf"
