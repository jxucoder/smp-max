...
ledger is [evidence ledger](results.md).

## 1. The result

Two explicit instances with 85 stable matchings each, [saved witnesses](../results/f7/witnesses-85.txt)
...
1. `tools.stable_matchings.stable_matchings` (brute force over all 5,040 permutations, the
...
Preference lists were checked to be permutations on both sides. Recount the
saved instances from the repository root:

```bash
python3 tools/verify_witnesses.py results/f7/witnesses-85.json
```

**Reproduce.** `experiments/search/search_swap_schedules.py` is `experiments/search/search_full_budget_swaps.py`
...
full budget of n(n-1)/2 transpositions. From the clone root:
    ...
    python3 experiments/search/search_swap_schedules.py 7 <seed> <seconds>          # free length
    python3 experiments/search/search_swap_schedules.py 7 <seed> <seconds> --full   # full budget (21 steps)
    python3 experiments/search/search_swap_schedules.py 6 0 25      # sanity: reaches 48 = f(6)
...
(`results/f7/search/free_s9_300s.log`, `results/f7/search/free_s5_300s.log`; re-derived
2026-09-08).

**The committed logs** (`results/f7/search/`, one core per run, 2026-09-08 rerun of
the unlogged 2026-09-06 runs) show:

- Free length (`results/f7/search/free_s*_{300,900}s.log`): at 300 s, **12 of 12
...
- Full budget (`results/f7/search/full_s0..9_300s.log`, `--full`: the length pinned
...
6 and the f(6) = 48 result is unaffected. `tools.stable_matchings.stable_matchings`
...
Conjecture 1 (`../papers/f6/f6-max-stable-matchings.pdf`): f(n) is attained by an all-size-2
...
formula of this shape is scaled again (campaign numbers: [evidence ledger](results.md)):

- **46% of the order-6 base CNF is a pairwise at-most-one** on the shape
  alphabet (the step one-hot block of `../tools/schedule_encoding.py`: 15 x C(410,2)
