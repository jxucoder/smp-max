...
(`../lean/SmpMax/*6.lean`: `validity_unconditional`, `bridge`,
...
see `../docs/verification.md`. The lower bound is the dihedral instance below.
...
`../lean/SmpMax/Six/ExactMaximum.lean`, zero sorries, standard axioms, so
f(6) = 48 is one Lean theorem whose only hypothesis is the certificates
(`../docs/results.md`). Paper:
...
- Lower bound: f(6) >= 48, attained by the dihedral-group Latin instance
  ...
  Our counter (`../tools/stable_matchings.py`) reproduces 48 stable matchings for it.
...
The faithfulness theorem is not yet proved; see `../docs/results.md`.
...
header of this file and `../docs/results.md`.)
