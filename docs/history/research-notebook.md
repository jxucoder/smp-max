...
verify the results (see `docs/verification.md`); it records how the evidence
chains were built, in the order they were built. Later notes:
`docs/reference/campaign.md` (the order-6 certificate campaign), `docs/f6.md`
(the order-6 attack log), `docs/f7.md`, `docs/history/research-retrospective.md`.
...
- n=5, k=16: SAT in 0.1s; decoded witness independently recounted at
  exactly 16 stable matchings (instance in `papers/f5/f5-max-stable-matchings.tex`).
...
total, mean solve 10.5s (`experiments/sat/run_direct_cubes.py` -> `f5/cubes/`, regenerable,
...
`results/f5/direct-encoding/cake-lpr-results.txt`). Machine side of the f(5)<=16 evidence chain is
...
3. kissat refuted each cube, drat-trim verified each proof, and the
   ...
   **120/120 `s VERIFIED UNSAT`** (`results/f5/lean-cubes/verification.txt`).
4. Lower bound: the 16-matching witness instance is a kernel-only Lean
   theorem (`lean/checks/FiveWitness.lean`, axioms = [propext]).
...
`experiments/enumeration/enumerate_f5_profiles.py`, `results/f5/profile-enumeration.txt`). The quotient 4,227,120/24 =
...
The schedule reduction (see `papers/notes/f6-schedule-reduction.pdf` and `papers/f6/f6-max-stable-matchings.pdf`):
...
`lean/SmpMax/{SixBridge,Lattice6,Chain6}.lean` — 1,849 lines,
...
(`docs/reference/campaign.md`), not on the enumeration above. The Lean-defined
...
after checking. `python3 tools/campaign/cube_campaign.py --audit` exits 0
...
released journal (`results/f6/campaign-2026-09-08/campaign.jsonl.gz`, 40,943,262 bytes,
...
Lean identity checks, all passed (`results/f6/campaign-2026-09-08/lean_identity.txt`):
...
unit clauses (`tools/campaign/verify_lean_hashes.py`, exit 0).
...
`docs/results.md` for its status.

## f(6) = 48 as one Lean theorem (2026-09-08, evening)

The faithfulness proof planned in `docs/design/f6-faithfulness.md` is complete.
`f6_eq_48_of_unsat` (`lean/SmpMax/Six/ExactMaximum.lean`): if every cube
