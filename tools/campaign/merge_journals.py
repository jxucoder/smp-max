...
and, within a class, the latest `ts`.  A `verified` record reports a checked
certificate for that cube's formula; the audit validates its recorded fields
(verify_record), not certificate contents. A `split` only promises its children, so it must never
