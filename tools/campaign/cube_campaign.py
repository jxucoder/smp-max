#!/usr/bin/env python3
"""Run or audit the order-6 cube-and-conquer certificate campaign.

The driver builds the schedule CNF, fixes canonical prefixes, solves each
cube, checks its certificate with cake_lpr, and records hashes/verdicts.
Adaptive splits extend a prefix with every legal canonical next step and
the stopped case. The mathematical counterexample-to-cube theorem is still
in progress; see docs/results.md and docs/design/f6-faithfulness.md.

The completed journal is immutable evidence under results/f6/. New runs
default to runs/f6/. An audit checks recorded coverage/verdicts and formula
identity; it does not recheck deleted LRAT contents. Encoder provenance
fields retain the historical names sched_sat_path and sched_sat_sha256.
...
Usage from the repository root:
  ...
  python3 -m tools.campaign.cube_campaign --solver cadical --workers 4

See docs/reference/campaign.md for setup, resource estimates, resume,
sharding, SAT handling, and optional comparisons to preexported Lean CNFs."""
...
TOOLCHAIN_ROOT = os.environ.get("SMP_TOOLCHAIN_ROOT", ROOT)
RUNS = os.path.join(ROOT, "runs", "f6")
from tools.schedule_encoding import build, cyclic_shapes, decode, readoff_counts  # noqa: E402
from tools.rotation_poset import stable_matchings  # noqa: E402

N = 6
K = 49
BUDGET = N * (N - 1)          # total move budget (30)
PERMAN = N - 1                # per-man cap (5)
DT = os.path.join(TOOLCHAIN_ROOT, "dt-src", "drat-trim")
CAKE = os.path.join(TOOLCHAIN_ROOT, "cake_lpr-src", "cake_lpr")
KISSAT = shutil.which("kissat") or "kissat"
CADICAL = os.path.join(TOOLCHAIN_ROOT, "cadical-src", "build", "cadical")
SOLVERS = ("kissat", "cadical")
SCHED_SAT = os.path.join(ROOT, "tools", "schedule_encoding.py")
LEAN_EXPORT = os.path.join(ROOT, "lean", ".lake", "build", "bin",
                           "export_sched_cnf")
...
def audit(recs, depth=2, headers=(), expect_dir=None):
    ...
    for h in headers:
        ...
        if h.get("sched_sat_sha256") != _G["sched_sat_sha256"]:
            problems.append("header sched_sat_sha256 != current schedule_encoding.py "
...
def main():
    ...
    ap.add_argument("--scratch", default=os.path.join(RUNS, "scratch"))
    ap.add_argument("--keep-dir", default=os.path.join(RUNS, "keep"))
    ...
    for h in headers:
        ...
        if h.get("sched_sat_sha256") != _G["sched_sat_sha256"]:
            print("warning: schedule_encoding.py changed since the journal header "
