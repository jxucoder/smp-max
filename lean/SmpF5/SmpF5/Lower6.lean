...
Implements `docs/history/PLAN-2026-09-08.md` §A (file `Lower6.lean`) and
`docs/history/f6-FAITHFULNESS_PLAN.md` §7 L7.4 (the dihedral lower bound needed by
...
Conventions (as in `latin_instance` of `smp.py`): `mrank[i][j] = entry - 1`
and woman `j` gives man `i` the rank `7 - entry`, i.e.
`wrank[j][i] = 6 - entry`.  Both tables are written out explicitly below;
the repository's brute-force counter (`smp.py`) gives 48 stable matchings
for this matrix.
