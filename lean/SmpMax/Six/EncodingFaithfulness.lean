...
* `exists_canonical_schedule` (the witness chain of plan §1/§3.4 and
  `docs/history/research-plan-2026-09-08.md` A): from `49 ≤ stableCount6 I` to a legal, first-appearance
...
extra import, `SmpMax.Six.CampaignSplits`, because `Cubes6.finalCubes` (used only in
`f6_upper_of_unsat_of_coverage`) is defined there; the module is built and
already imported by `SmpMax.lean`.
-/
...
/-! ## The witness chain (plan §1, §3.4; `docs/history/research-plan-2026-09-08.md` A) -/
...
/-- Plan §1/§3.4, `docs/history/research-plan-2026-09-08.md` A: every well-formed instance with at least 49
