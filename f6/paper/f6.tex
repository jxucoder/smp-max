...
The bridge and validity lemmas are machine-checked in Lean~4 (no \texttt{sorry}, standard
axioms); the symmetry lemma is validated computationally, and a verified replay of the
enumeration remains future work.
\end{abstract}
...
the accompanying development ($\sim$4{,}600 lines across nine files, from
\texttt{SixBridge.lean} through \texttt{WRelabel6.lean}, atop the order-5 infrastructure
of~\cite{f5paper}). The end statement is unconditional: for every well-formed order-6
...
is \emph{not} formalized: its relabelling half is proved at the instance level
(\texttt{Sym6.lean}), its commutation half is validated computationally only, and the
certification campaign of Section~\ref{sec:outlook} is designed to need neither
(canonical first-appearance cubes are generated without the commutation rule). Every
theorem is proved with no \texttt{sorry}, and \texttt{\#print axioms} reports only the
standard \texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}; continuous
integration rebuilds the development and re-checks these claims on every commit.
