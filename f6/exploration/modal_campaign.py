...
sys.path.insert(0, os.path.join(HERE, ".."))  # f6/: cube_campaign.py, sched_sat.py
...
if modal is not None:
    ...
    image = (
        ...
        # the campaign modules land in /root, so cube_campaign's HERE/.. = /
        .add_local_python_source("cube_campaign", "sched_sat", "rotation_poset", "cube_calibrate")
    )
...
def drive(args, runner, info):
    ...
    if headers and headers[0].get("base_sha256") != cc._G["base_sha256"]:
        sys.exit("journal header base_sha256 != this sched_sat.py's formula; refusing")
...
def local_runner(batch, cfg):
    for cid in batch:
        try:
            yield cid, run_in_container(cid, cfg, scratch=os.path.join(HERE, "campaign", "scratch_modal_local"))
...
if modal is not None:
    ...
    def main(journal: str = os.path.join(HERE, "campaign", "campaign.jsonl"), sample: int = 0,
